...
function DoCMake () {
    ...
    cd    $root/build_$1$2
    echo "cmake ../src/cmake -G "$generator" -B "." -D CONFIGURATION_TYPE="$1" -D CMAKE_BUILD_TYPE=${buildtype} -S ../src/cmake &>build_$1$2/cmake_$1.log"
          cmake ../src/cmake -G "$generator" -B "." -D CONFIGURATION_TYPE="$1" -D CMAKE_BUILD_TYPE=${buildtype} \
         -DGDAL_LIBRARY=$GDALDIR/lib/libgdal.so -DGDAL_INCLUDE_DIR=$GDALDIR/include \
         -DPROJ_DIR=$PROJDIR/lib64/cmake/proj \
		 -S ../src/cmake &>cmake_$1.log
    if [ $? -ne 0 ]; then
...
function BuildCMake () {
    ...
    echo "make VERBOSE=1 install &>build_$1$2/make_$1.log"
          ...
          #make VERBOSE=1
          #make install
          #make install throwing ELF errors
    if [ $? -ne 0 ]; then
        echo "CMake build resulted in an error. Check log files."
        exit 1
    fi
    cp ./build_all/dimr_lib/libdimr_lib.so ./build_all/install/lib/
...
if [ "$isdeltares" = "yes" ]; then
    ...
    # On SANDIA-chama systems only:
    echo ". $root/setenv-chama.sh $compiler"
          . $root/setenv-chama.sh $compiler

    ## parallel build (SANDIA)
    nproc=128
...
# comment out these 2 lines to rebuild without starting fresh
