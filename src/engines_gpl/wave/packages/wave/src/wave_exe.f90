...
!  $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/trunk/src/engines_gpl/wave/packages/wave/src/wave_exe.f90 $
!!--description-----------------------------------------------------------------
! NONE
   ...
   if (wavedata%mode /= stand_alone) then
      ...
      do while (command == flow_wave_comm_perform_step)
         ...
         if (timtscale >= 0) then
            ...
            !
            call swan_tot(n_swan_grids, n_flow_grids, wavedata, -1)
         else
