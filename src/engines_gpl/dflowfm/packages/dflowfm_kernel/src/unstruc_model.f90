...
subroutine readMDUFile(filename, istat)
    ...
    real(kind=hp) :: hkad
    real(kind=hp) :: ti_rst_array(3), ti_map_array(3), ti_his_array(3), acc, ti_wav_array(3), ti_waq_array(3)
    real(kind=sp) :: rtmp
    character(len=200), dimension(:), allocatable       :: fnames
...
!    Secondary Flow 
    ...
    call prop_get_integer(md_ptr, 'sediment', 'Sedimentmodelnr'    ,  jased) ! 1 = krone, 2 = svr, 3 engelund
...
subroutine writeMDUFilepointer(mout, writeall, istat)
    ...
    integer                        :: i
    real(kind=hp)                  :: ti_wav_array(3), ti_map_array(3), ti_rst_array(3), ti_his_array(3), ti_waq_array(3)
...
! output to mdu file
    ...
    call prop_set(prop_ptr,    'wind', 'ICdtyp',               ICdtyp,   'Wind drag coefficient type (1=Const; 2=Smith&Banke (2 pts); 3=S&B (3 pts); 4=Charnock 1955, 5=Whang 2005, 6=Wuest 2005, 7=Hersbach 2010 (2 pts)' )
   ...
   ! JRE -> aanvullen, kijken wat aangeleverd wordt 
    ...
    if (ja_timestep_auto .ne. 1 .and. ja_timestep_auto .ne. 5) then 
        call prop_set(prop_ptr, 'time', 'AutoTimestep',  ja_timestep_auto, '0 = no, 1 = 2D (hor. out), 3=3D (hor. out), 5 = 3D (hor. inout + ver. inout')
    endif
...
!    call prop_set(prop_ptr, 'output', 'WaqFileBase', trim(md_waqfilebase), 'Basename (without extension) for all Delwaq files to be written.')
  ...
  ! call prop_set(prop_ptr, 'output', 'SnapshotDir', trim(md_snapshotdir), 'Directory where snapshots/screendumps are saved.')
    ...
    if (writeall .or. jahissourcesink > 0) then
    ...
    if (writeall .or. jahisvelvec > 0) then
    ...
    if(jacali > 0 .and. (writeall .or. jamapcali > 0)) then
   ...
   do while (ja .eq. 1)                                ! read *.ext file
      ...
      if (qid(L1:L2) == 'bnd') then
         ...
         case ('frictioncoefficient', 'frictiontrtfactor', 'horizontaleddyviscositycoefficient', 'horizontaleddydiffusivitycoefficient', 'advectiontype', &
               'bedlevel', 'ibedlevtype', 'initialwaterlevel', 'initialsalinity', 'initialsalinitytop', 'initialsediment', &
               'initialverticaltemperatureprofile', 'initialverticalsalinityprofile', 'windstresscoefficient', & 
               'stemdiameter', 'stemdensity', 'stemheight' , 'interceptionlayerthickness')
            qtype = 'initial'
