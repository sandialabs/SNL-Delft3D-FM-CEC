            ...
            do ij = 1, size(gdp%turbines%nr)	
              ...
              if (turbine%turbulencemodel==1) then  ! Rethore model	
                ...
                do n = 1,size(turbine%edgelist)	
                  ...
                  do k = 1, kmax	
                    uuu = u1(nm,k) !sqrt(u1(nm, k)**2+v1(nm, k)**2)	
                    !ddk(nm, k) = ddk(nm, k) + Cx*( turbine%beta_p*uuu**3 - turbine%beta_d*uuu*rtur0(nm,k,1) )  &	
                    !                            *turbine%blockfrac(n, k)/gvu(nm)	
                    bbk(nm, k) = bbk(nm, k) - Cx*( - turbine%beta_d*uuu )*turbine%blockfrac(n, k)/gvu(nm)	
                    ddk(nm, k) = ddk(nm, k) + Cx*( turbine%beta_p*uuu**3 )*turbine%blockfrac(n, k)/gvu(nm)	
                  enddo	
                enddo	
              ...
              if (turbine%turbulencemodel==1) then  ! Rethore model	
                ...
                do n = 1,size(turbine%edgelist)	
                  ...
                  do k = 1, kmax	
                    ...
                    bbk(nm, k) = bbk(nm, k) - Cx*(  turbine%cep4*turbine%beta_p/rtur1(nm,k,1)*uuu**3  &	
                                                  - turbine%cep5*turbine%beta_d*uuu  ) *turbine%blockfrac(n, k)/gvu(nm)	
                  enddo	
                enddo	
