...
! 
! Usage of 0/1-arrays  (e.g. S0 and S1 for the water levels at the beginning 
!                      and end of a half time step):
!                      Up to and including the call to Z_ADI (solution of the 
!                      coupled momentum and continuity equations), the 0-arrays
!                      are used.
!                      After Z_ADI, the new layering administration has been
!                      updated and all 1-arrays should be used.
!                      EXCEPTION: routines related to transport of constituents
!                      use the old geometry as the R0 array is also only
!                      available at this level. Special measures to update the
!                      concentrations in newly activated layers or cells are
!                      taken in Z_DIFU and in HEATU for implicit treatment of 
!                      sinks.
!                      Routines where this is the case are Z_DISCHA, HEATU, 
!                      THAHBC.
!                      Routine Z_DIFUFLUX requires different treatment
!                      based on the selected advection option
!                      (explicit/implicit) for constituent transport.
!
...
!
    ...
    if (.not. dryrun) then
       ...
       call z_adi(stage     ,jstart    ,nmmaxj    ,nmmax     ,kmax      , &
                ...
                & r(dzv0)   ,r(dzs0)   ,r(dzu1)   ,r(dzv1)   ,r(dzs1)   , &
                & r(qxk)    ,r(qyk)    ,r(qzk)    ,r(evap)   ,r(circ2d) , &
                ...
                & r(windsv) ,r(patm)   ,r(fcorio) ,r(wrkb17) ,r(wrka1)  , &
                & r(wrka2)  ,r(wrka3)  ,r(wrka4)  ,r(wrka5)  ,r(wrka6)  , &
                & r(wrka7)  ,r(wrka8)  ,r(wrka15) ,r(wrka16) ,r(wrkb1)  , &
       ...
       !
       ! 3D Turbulence (NOT HERE ONLY ONCE FOR ZMODEL)
       ! user defined BC for turbulence model in NUBND and UBND
       !
       !
       ! 2D Turbulence; BC already read in RDQ2EB (see READMD); ONLY ONCE ?
       ...
       ! Check is done based upon old geometry (corresponding to S0)
       !
       icx = nmaxddb
       icy = 1
    ...
    if (nto > 0) then
       ...
       if (sbkol) then
          ...
          !
          call timer_start(timer_wait, gdp)
          write(*,*) '2nd half time step: d3s_get_discharges'
          call d3s_get_discharges(ntstep, nto, kcd, r(hydrbc))
          write(*,*) '2nd half time step: d3s_get_discharges DONE'
          call timer_stop(timer_wait, gdp)
       endif
    endif
    !
    ...
    if (.not. dryrun) then
       ...
       !
       if (nonhyd) then
          call timer_start(timer_nonhyd, gdp)
          icx = nmaxddb
          icy = 1
          call z_vermom(nmmax     ,kmax      ,icx       ,icy       ,r(u0)     , &
                      & r(v0)     ,r(w0)     ,r(vicww)  ,r(rxz)    ,r(ryz)    , &
                      & r(guu)    ,r(gvv)    ,r(guv)    ,r(gvu)    ,i(kfs)    , &
                      & i(kcs)    ,r(wrkb1)  ,r(wrkb2)  ,r(wrkb3)  ,r(wrkb4)  , &
                      & i(kfuz1)  ,i(kfvz1)  ,i(kfsz1)  ,i(kfsmin) ,i(kfsmax) , &
                      & i(kcshyd) ,r(dzs1)   ,r(dzu0)   ,r(dzv0)   ,r(w1)     , &
                      & r(p0)     ,gdp       )
          !
          ! non-hydrostatic pressure
          !
          icx = nmaxddb
          icy = 1
          call z_hydpres(mmax      ,nmax      ,jstart    ,nmmaxj    ,nmmax     , &
                       & kmax      ,nst       ,icx       ,icy       ,nsrc      , &
                       & norow     ,nocol     ,i(irocol) ,r(s1)     ,r(s00)    , &
                       & r(u1)     ,r(v1)     ,r(w1)     ,r(guu)    ,r(gvv)    , &
                       & r(guv)    ,r(gvu)    ,r(gsqs)   ,r(guz)    ,r(gvz )   , &
                       & i(kfu)    ,i(kfv)    ,i(kfs)    ,i(kcs)    ,i(kfuz1)  , &
                       & i(kfvz1)  ,i(kfsz1)  ,i(kfsmin) ,i(kfsmax) ,i(kcshyd) , &
                       & i(mnksrc) ,r(umean)  ,r(vmean)  ,d(dps)    ,                       &
                       & r(dzu0)   ,r(dzv0)   ,r(dzs1)   ,r(disch)  ,            &
                       & r(p1)     ,r(p00)    ,r(pnhcor) ,r(wrkb1)  ,            &
                       & r(wrkb2)  ,r(wrkb3)  ,r(wrkb4)  ,r(wrkb5)  ,r(wrkb6)  , &
                       & r(wrkb7)  ,r(wrkb8)  ,r(wrkb9)  ,r(wrkb10) ,r(wrkb11) , &
                       & r(wrkb12) ,r(wrkb13) ,r(wrkb14) ,gdp       )
          !
          ! Correct the momentum
          !
          icx = nmaxddb
          icy = 1
          call z_momcor(nmmax     ,kmax      ,icx       ,icy       ,r(s1)     , &
                      & r(u1)     ,r(v1)     ,r(w1)     ,r(umean)  ,r(vmean)  , &
                      & r(qxk)    ,r(qyk)    ,r(qzk)    ,r(guu)    ,r(gvv)    , &
                      & r(guv)    ,r(gvu)    ,r(gsqs)   ,i(kcs)    ,i(kcshyd) , &
                      & i(kfuz1)  ,i(kfvz1)  ,i(kfsz1)  ,i(kfumin) ,i(kfumx0) , &
                      & i(kfvmin) ,i(kfvmx0) ,i(kfsmin) ,i(kfsmax) ,r(dzs1)   , &
                      & r(dzu0)   ,r(dzv0)   ,r(p1)     ,r(p0)     ,r(s00)    , &
                      & r(w0)     ,r(s0)     ,r(disch)  ,            &
          ...
          ! itemp has bendefined to call Z_DRYCHK (and set to zero) replacing idry
          !
          call z_drychk(itemp     ,jstart    ,nmmaxj    ,nmmax     ,kmax      , &
                      & nfltyp    ,icx       ,icy       ,i(kfu)    ,i(kfv)    , &
                      & i(kfs)    ,i(kcs)    ,i(kfuz1)  ,i(kfvz1)  ,i(kfsz1)  , &
          ...
          if (ztbml) then
             !
             ! Call with modify_dzsuv set to 1 only for the first component, to modify dzs1 only
             ! (and possibly R1 and qzk)
             !
             modify_dzsuv(1)   = 1
             modify_dzsuv(2:3) = 0
