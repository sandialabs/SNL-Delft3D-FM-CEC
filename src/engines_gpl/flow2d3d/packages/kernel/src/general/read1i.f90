subroutine read1i(record    ,lrec      ,ibeg      ,iend      ,ivar      , &
                & idfaul    ,ier       )
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
!                                                                               
!-------------------------------------------------------------------------------
!  $Id: read1i.f90 65778 2020-01-14 14:07:42Z mourits $
!  $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal_v3/src/engines_gpl/flow2d3d/packages/kernel/src/general/read1i.f90 $
!!--description-----------------------------------------------------------------
!
!    Function: Reads ONE integer variable from an arbitrary
!              string/record.
...
!              - If [] found, then if   DEFAULT IVAR=IDFAUL
!                                  else         IER =-1
!                             program is returned
!              - Otherwise read integer after setting the format
!
...
!
    ...
    integer     , intent(out) :: ivar   !!  Integer var. or array to store the value(s) read from a record
...
!
   ...
   20 continue
    ...
    if ((record(ibeg:ibeg)=='[') .or. (record(ibeg:ibeg)==']')) then
       ...
       read (record(ibeg:iend), fmti, iostat = iocond) ivar
       if (iocond/=0) then
          ier = -1
          goto 999
       endif
