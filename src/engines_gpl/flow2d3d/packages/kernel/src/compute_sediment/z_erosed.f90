subroutine z_erosed(nmmax     ,kmax      ,icx       ,icy       ,lundia    , &
...
!                                                                               
!-------------------------------------------------------------------------------
!  $Id: z_erosed.f90 7992 2018-01-09 10:27:35Z mourits $
!  $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal/src/engines_gpl/flow2d3d/packages/kernel/src/compute_sediment/z_erosed.f90 $
!!--description-----------------------------------------------------------------
!
...
!                srcmax(nm, l) = bodsed(l, nm)*cdryb(l)/dtmor
    ...
    do l = 1, lsed
       ...
       do nm = 1, nmmax
          ...
          !
          sour(nm, k, ll) = sour(nm, k, ll) + sourse(nm,l) * gsqs(nm) * dzs0(nm,kmxsed(nm,l))
          sink(nm, k, ll) = sink(nm, k, ll) + sinkse(nm,l) * gsqs(nm) * dzs1(nm,kmxsed(nm,l))
          if (iflufflyr>0) then
