...
!                                                                               
!-------------------------------------------------------------------------------
!  $Id: dfsendr_nm_pos2.F90 65778 2020-01-14 14:07:42Z mourits $
!  $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal_v3/src/engines_gpl/flow2d3d/packages/data/src/parallel_mpi/dfsendr_nm_pos2.F90 $
!!--description-----------------------------------------------------------------
!
...
!
    ...
    real(fp), dimension(ks:ke,gdp%d%nmlb:gdp%d%nmub), intent(in)    :: field        ! real array for which halo values must
...
!
    ...
    do inb = 1, nneigh
       ...
       !
#ifdef HAVE_MPI
       call mpi_isend ( work(1,inb,1), novlu*ksiz, MPI_REAL, idom-1, itag, engine_comm_world, request(inb,1), ierr )
       ...
       call mpi_irecv ( work(1,inb,2), novlu*ksiz, MPI_REAL, idom-1, itag, engine_comm_world, request(inb,2), ierr )
       if ( ierr /= MPI_SUCCESS ) then
          write (msgstr,'(a,i5,a,i3.3)') 'MPI produces some internal error - return code is ',ierr,' and node number is ',inode
          call prterr(lundia, 'U021', trim(msgstr))
          call d3stop(1, gdp)
...
#endif
    enddo
end subroutine dfsendr_nm_pos2
