...
!                                                                               
!-------------------------------------------------------------------------------
!  $Id: read_src_tmp.f90 7992 2018-01-09 10:27:35Z mourits $
!  $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal/src/utils_lgpl/hyd_to_waq_part_lib/packages/hyd_to_waq_part_lib/src/read_src_tmp.f90 $
      ...
      inpfil%inplun(1) = file_src_tmp%unit_nr
      inpfil%finame(1) = file_src_tmp%name
      inpfil%cchar  = ';'
      inpfil%iposr  = 0
      ...
      do
         ...
         if(ierr.ne.0 .or. int.ge.first_itime_current) then
            exit
         end if
         ! dummy read the data
         do iwaste = 1 , no_flow
      ...
      do ibrk = 1 , nobrk_waste_tmp
         call dlwq_read_token( inpfil, wasteload_data_tmp%times(ibrk), ierr)
         if ( ierr .ne. 0 ) then
            write(lunrep,*) ' error reading sources file'
