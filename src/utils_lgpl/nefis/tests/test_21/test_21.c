...
//
//------------------------------------------------------------------------------
// $Id: test_21.c 65778 2020-01-14 14:07:42Z mourits $
// $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal_v3/src/utils_lgpl/nefis/tests/test_21/test_21.c $
//
//------------------------------------------------------------------------------
// Functionaliteit restart va map-fle en verschillend aantal stations op history file
// Hier testen het verschillend aanatal stations op verschillende NEFIS files, tegeljkerijd open
// Open 2 nefis met verschillend aantal stations
//------------------------------------------------------------------------------
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
int main()
{
  ...
  strcpy(dat_filea, "data_c21a.dat");
  strcpy(def_filea, "data_c21a.def");
  strcpy(dat_fileb, "data_c21b.dat");
  strcpy(def_fileb, "data_c21b.def");
  strcpy(dat_filec, "data_c21c.dat");
  strcpy(def_filec, "data_c21c.def");
...
/*-------------------------------------------------------------------------*/
  ...
  strcpy( elm_names[0],"wl"  );
  if (error == 0 ) {
    printf(" Define first cel     \n");
    error  = Defcel2( &fd_nefisa, cel_name, cel_num_dim , elm_names);
    error  = Defcel2( &fd_nefisb, cel_name, cel_num_dim , elm_names);
  }

  cel_num_dim = 2;
  strcpy( cel_name    ,"cel2");;
  strcpy( elm_names[0], "wl");
  strcpy( elm_names[1], "names");
...
/*-------------------------------------------------------------------------*/
  ...
  rdwr = 'u';
  error = Crenef( &fd_nefisc, dat_filec, def_filec, coding,  rdwr);
  strcpy(elm_name, "wl");
  strcpy(elm_type, "INTEGER");
  strcpy(elm_quantity, "Waterstanden");
  strcpy(elm_unity, "meter");
  ...
  if (error == 0) {
	  printf(" Define first element \n");
	  error = Defelm(&fd_nefisc, elm_name, elm_type, elm_single_byte,
		  elm_quantity, elm_unity, elm_desc,
		  elm_num_dim, elm_dimens);
  }
  ...
  if (error == 0) {
	  printf(" Define first cel     \n");
	  error = Defcel2(&fd_nefisc, cel_name, cel_num_dim, elm_names);
  }
  ...
  if (error == 0) {
	  printf(" Define first group   \n");
	  error = Defgrp(&fd_nefisc, grp_name, cel_names, grp_num_dim,
		  grp_dimens, grp_order);
  }
  ...
  if (error == 0) {
	  ...
	  strcpy(grp_name, "group1");
	  strcpy(grp_defined, "grp1");
	  error = Credat(&fd_nefisc, grp_name, grp_defined);
  }
  ...
  /*-------------------------------------------------------------------------*/
  if (error == 0) error = Clsnef(&fd_nefisc);
  if (error == 0) error = Clsnef(&fd_nefisa);
  /*-------------------------------------------------------------------------*/
  error = Crenef(&fd_nefisb, dat_fileb, def_fileb, coding, rdwr);
