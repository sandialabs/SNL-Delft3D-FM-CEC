...
!                                                                               
!-------------------------------------------------------------------------------
!  $Id: tst-2d-put.F90 65778 2020-01-14 14:07:42Z mourits $
!  $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal_v3/src/utils_lgpl/delftio/tests/tstAll2D/src/tst-2d-put.F90 $
...
!   Put data for each timestop
    ...
    do i = 1, NTIMES
        ...
        call Dio2DFPut(set(ra),rvalues)
        call Dio2DFPut(set(da),dvalues)
        call Dio2DFPut(set(ia),ivalues)

        if (.not. auto) then
            call Dio2DFPut(set(rb),rvalues)
            call Dio2DFPut(set(db),dvalues)
            call Dio2DFPut(set(ib),ivalues)
        endif
