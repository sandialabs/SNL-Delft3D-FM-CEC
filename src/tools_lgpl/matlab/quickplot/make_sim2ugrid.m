...
%   $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/tags/delft3dfm/141476/src/tools_lgpl/matlab/quickplot/make_sim2ugrid.m $
%   $Id: make_sim2ugrid.m 140618 2022-01-12 13:12:04Z klapwijk $

curdir=pwd;
addpath(curdir)
if matlabversionnumber<7.09
    error('Invalid MATLAB version. Use MATLAB R2009b (7.9) or higher for compiling Delft3D-SIM2UGRID!')
end
if ~exist('mcc')
...
fprintf('\nBuilding Delft3D-SIM2UGRID version %s\n\n',qpversion);
TStr = datestr(T);
fstrrep('sim2ugrid.m','<VERSION>',qpversion)
fstrrep('sim2ugrid.m','<CREATIONDATE>',TStr)
g = which('-all','gscript');
if ~isempty(g)
    copyfile(g{1},'.')
end
%
mcc('-m','-a','./units.ini','-a','./grib','-v','sim2ugrid.m')
%
X={'*.asv'
    '*.bak'
    '*.m'
