...
%   $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal/src/tools_lgpl/matlab/quickplot/progsrc/vs_copy.m $
%   $Id: vs_copy.m 7992 2018-01-09 10:27:35Z mourits $
            ...
            for e=1:length(gElms{i0})
                ...
                if VD
                    for k=1:length(VDIdx)
                        ...
                        Idx{VD}=VDIdx(k);
                        if strcmp(VS.FileName,'IN MEMORY')
                            gName_ = strrep(gName{i0},'-','_');
                            eName_ = strrep(eName,'-','_');
                            Data=VS.Data.(gName_).(eName_); % no support for Idx yet
                        else
