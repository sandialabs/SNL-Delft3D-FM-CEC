...
%   $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal_v3/src/tools_lgpl/matlab/quickplot/progsrc/qp_validate.m $
%   $Id: qp_validate.m 65939 2020-02-06 15:13:11Z jagers $
...
switch log_style
    case 'latex'
        if strcmp(casename,'MAIN')
            ...
            fn = fopen(logid);
            [~,f,e] = fileparts(fn);
            fprintf(logid,'%s%d%s\n',['\svnid{$Id: qp_validate.m 65939 2020-02-06 15:13:11Z jagers $}']);
            fprintf(logid,'\n');
            fprintf(logid,'%s\n','\begin{document}');
            fprintf(logid,'%% %s\n','\pagestyle{empty}');
