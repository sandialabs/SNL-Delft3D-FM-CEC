function make_delwaq2raster(basedir,varargin)
%MAKE_DELWAQ2RASTER Compile delwaq2raster executable
%   Compile MATLAB code to delwaq2raster executable
%
%   MAKE_DELWAQ2RASTER(BASEDIR)
...
%   $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/research/SANDIA/fm_tidal/src/tools_lgpl/matlab/quickplot/make_delwaq2raster.m $
%   $Id: make_delwaq2raster.m 7992 2018-01-09 10:27:35Z mourits $

curdir=pwd;
addpath(curdir)
if matlabversionnumber<7.09
    error('Invalid MATLAB version. Use MATLAB R2009b (7.9) or higher for compiling Delft3D-DELWAQ2RASTER!')
end
if ~exist('mcc')
...
if strncmp(fliplr(computer),'46',2)
    exedir = 'delwaq2raster64';
else
    exedir = 'delwaq2raster32';
end
...
cd(exedir);
diary make_delwaq2raster_diary
if isunix
    unix('cp -rf ../progsrc/* .');
    unix('mv compileonly/* .');
else
